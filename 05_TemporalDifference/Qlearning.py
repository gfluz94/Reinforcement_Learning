...
class Robot():
    ...
    def go_through_episode(self, env, alpha):
        current_position = env.start
        self.state_history.append(current_position)
        action = self.next_action(env, current_position, epsilon=1)
        gamma = 0.9
        while not env.game_over():
            next_state = self.get_next_state(action, current_position, env, self.probability_action)
            r = env.rewards[next_state]
            next_action = self.next_action(env, next_state, epsilon=1)
            next_action_Q = self.next_action(env, next_state)
            new_alpha = alpha/self.N[current_position][action]
            self.Q[current_position][action] += new_alpha*(r+gamma*self.Q[next_state][next_action_Q]-self.Q[current_position][action])
...
if __name__ == "__main__":
    ...
    for i in range(episodes):   
        ...
        robot.go_through_episode(env, alpha)
